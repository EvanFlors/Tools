...
describe("PaymentService", () => {
  ...
  test("should delete payment from completed sale and revert to active", async () => {
    const userId = id();
    const sale = await createSale(500);

    // Pay the full amount so the sale becomes completed
    const payment = await PaymentService.create({
      saleId: sale._id,
      amount: 500,
      userId,
    ...
    expect(completedSale.remainingBalance).toBe(0);

    // Delete the payment — should succeed and revert the sale
    const result = await PaymentService.deleteById(payment._id, userId);

    const revertedSale = await Sale.findById(sale._id);
    expect(revertedSale.remainingBalance).toBe(500);
    expect(revertedSale.status).toBe("active");
    expect(result.message).toBe("Payment deleted successfully");
  });

  test("should throw when deleting non-existing payment", async () => {
    await expect(PaymentService.deleteById(id(), id())).rejects.toThrow(
      "Payment not found"
    );
  });
  ...
  test("should reject invalid payment update", async () => {
    ...
    await expect(
      PaymentService.updateById(payment._id, { amount: 1000 }, id())
